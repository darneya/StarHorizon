# Base entries
guide-entry-nf14 = Руководство Фронтира
guide-entry-bank = Галактический банк NT
guide-entry-piloting = Пилотирование
guide-entry-startinggear = Стартовое снаряжение
guide-entry-hiring = Наем экипажа
guide-entry-interviews = Интервью
guide-entry-expeditions = Экспедиции
guide-entry-sector-topology = Топология сектора
guide-entry-cargo-hauling = Грузоперевозки
guide-entry-shipyard = Верфь
guide-entry-frontier-rules = Правила сервера
guide-entry-frontier-definitions = Определения
guide-entry-preflightchecklist = Предполетная подготовка
# Engineering entries
guide-entry-offshore-gas-mining = Газодобыча
# Security entries
guide-entry-nfsd-smuggling = Контрабанда
# Medical entries
guide-entry-medical-bounties = Медицинские выплаты
guide-entry-medical-doc = Менеджер медицинских операций
# Reference entries
guide-entry-medicalrecipes = Медицинские рецепты
guide-entry-weapons-ranged-archetypes = Архетипы дальнебойного оружия
guide-entry-weapons-color-codes-ammo = Цветовые коды боеприпасов
# Expedition faction entries
guide-entry-expedition-aberrant-flesh = Аберрантная плоть
guide-entry-expedition-argocytes = Аргоциты
guide-entry-expedition-blood-cultists = Культисты
guide-entry-expedition-carps = Карпы
guide-entry-expedition-dinosaurs = Динозавры
guide-entry-expedition-explorers = Исследователи
guide-entry-expedition-mercenaries = Наёмники
guide-entry-expedition-gangers = Панки
guide-entry-expedition-silicons = Роботы-изгои
guide-entry-expedition-syndicate-naval-forces = Синдикат
guide-entry-expedition-xenos = Ксено
# Shipyard entries
guide-entry-shipyard-adder = Аддер
guide-entry-shipyard-aegis = Эгида
guide-entry-shipyard-akupara = Акупара
guide-entry-shipyard-ambition = Амбициозный
guide-entry-shipyard-anchor = Якорь
guide-entry-shipyard-apothecary = Апотекарий
guide-entry-shipyard-barge = Баржа
guide-entry-shipyard-baroness = Баронесса
guide-entry-shipyard-bazaar = Барахолка
guide-entry-shipyard-bocadillo = Фудтрак
guide-entry-shipyard-bodkin = Игуана
guide-entry-shipyard-bookworm = Книжный червь
guide-entry-shipyard-broom = Швабра
guide-entry-shipyard-brigand = Разбойник
guide-entry-shipyard-bulker = Балкер
guide-entry-shipyard-caladrius = Каладрий
guide-entry-shipyard-camper = Кемпер
guide-entry-shipyard-ceres = Церера
guide-entry-shipyard-charon = Харон
guide-entry-shipyard-chisel = Зубило
guide-entry-shipyard-comet = Комета
guide-entry-shipyard-courier = Курьер
guide-entry-shipyard-construct = Каркас
guide-entry-shipyard-crescent = Полумесяц
guide-entry-shipyard-copperhead = Медянка
guide-entry-shipyard-dove = Голубь
guide-entry-shipyard-dragoon = Драгун
guide-entry-shipyard-dragonfly = Стрекоза
guide-entry-shipyard-crossroads = Перекресток
guide-entry-shipyard-druid = Друид
guide-entry-shipyard-firefighter = Пожарный
guide-entry-shipyard-eagle = Орлан
guide-entry-shipyard-garden = Розарий
guide-entry-shipyard-gasbender = Газовый Гигант
guide-entry-shipyard-gaslight = Газлайт
guide-entry-shipyard-gourd = Цукини
guide-entry-shipyard-geode = Геода
guide-entry-shipyard-hammer = Наковальня
guide-entry-shipyard-hauler = Тягач
guide-entry-shipyard-harbormaster = Квадрон
guide-entry-shipyard-hound = Собака
guide-entry-shipyard-honker = ХОНКЕР
guide-entry-shipyard-investigator = Исследователь
guide-entry-shipyard-izakaya = Изакая
guide-entry-shipyard-kestrel = Пустельга
guide-entry-shipyard-lantern-mk2 = Кадило Mk2
guide-entry-shipyard-kilderkin = Бочка
guide-entry-shipyard-stronghold = Крепость
guide-entry-shipyard-stringer = Струнник
guide-entry-shipyard-lantern = Кадило
guide-entry-shipyard-legman = Интервьюер
guide-entry-shipyard-liquidator = Ликвидатор
guide-entry-shipyard-loader = Такелажник
guide-entry-shipyard-lyrae = Лира
guide-entry-shipyard-mccargo = Каргония
guide-entry-shipyard-nightlight = Ночник
guide-entry-shipyard-mcdelivery = Курьер
guide-entry-shipyard-nook = Уголок
guide-entry-shipyard-pathfinder = Следопыт
guide-entry-shipyard-phoenix = Феникс
guide-entry-shipyard-pioneer = Пионер
guide-entry-shipyard-placebo = Плацебо
guide-entry-shipyard-prospector = Старатель
guide-entry-shipyard-pulsar = Пульсар
guide-entry-shipyard-retort = Реторта
guide-entry-shipyard-searchlight = Прожектор
guide-entry-shipyard-sparrow = Воробей
guide-entry-shipyard-stellaris = Стелларис
guide-entry-shipyard-skipper = Шкипер
guide-entry-shipyard-spirit = Дух
guide-entry-shipyard-tyne = Тайн
guide-entry-shipyard-stasis = Стазис
guide-entry-shipyard-surveyor = Геодезист
guide-entry-shipyard-tide = Волна
guide-entry-shipyard-vagabond = Бродяга
guide-entry-shipyard-vitalis = Виталис
# Rules entries
guide-entry-frontier-rule-zero-tolerance = Нетерпимость
# Rules entries
guide-entry-frontier-rule-0 = Режим фронтира
guide-entry-frontier-rule-1 = 0. Не будь мудаком
guide-entry-frontier-rule-2 = 1. Нечестная игра
guide-entry-frontier-rule-3 = 2. Ценность жизни
guide-entry-frontier-rule-4 = 3. Метагейминг
guide-entry-frontier-rule-5 = 4. РП-отыгрыш
guide-entry-frontier-rule-6 = 5. Убийство, косвенное убийство, РДМ
guide-entry-frontier-rule-7 = 6. Неприемлемое поведение для занимаемой должности
guide-entry-frontier-rule-8 = 7. Мародерство
guide-entry-frontier-rule-9 = 8. Самоантагонизм
guide-entry-frontier-rule-10 = 9. Игра за пиратов
guide-entry-frontier-rule-11 = 10. Кража имущества
guide-entry-frontier-rule-13 = 13. Стандарты командования
guide-entry-frontier-rule-12 = 11. Злоупотреблеение АХелпом / Спам
