-nf-pirate-role-greeting-suffix = Выполняйте заказы Союза вольных стрелков в обмен на дублоны. Обойдите закон и сколотите целое состояние!
nf-pirate-role-greeting =
    Вы - пират на борту скрытой бухты в глубинах космоса.
    Помогите своему капитану разграбить сектор и { -nf-pirate-role-greeting-suffix }
nf-piratecaptain-role-greeting =
    Вы - капитан группы пиратов на скрытой бухте в глубинах космоса.
    Возглавьте свою команду, чтобы разграбить сектор и { -nf-pirate-role-greeting-suffix }
nf-pirate-role-greeting-equipment = Используйте консоли на вашей базе, чтобы найти и выкупить награду, а также продать ящики с контрабандой. Купите корабль и отправляйтесь навстречу приключениям!
objective-issuer-nf-pirate = [color=#F6CE64]Союз вольных стрелков[/color]
nf-pirate-round-end-agent-name = пират
roles-antag-nf-pirate-objective = Грабьте сектор, собирайте дублоны.
