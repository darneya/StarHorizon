...
ent-WeaponSubMachineGunC20rROW = C-20r ROW
    .desc = Пистолет-пулемет C-20r с режимом стрельбы очередями, предназначенный для использования киборгами. Создает патроны калибра .35 на ходу с помощью встроенного фабрикатора боеприпасов, который медленно самозаряжается.
