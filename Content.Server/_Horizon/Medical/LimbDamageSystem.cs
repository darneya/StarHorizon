...
public sealed class LimbDamageSystem : EntitySystem
{
    ...
    */
    ...
    //duct tape solution
    private static void OnDamage(Entity<BodyComponent> ent, ref DamageBeforeApplyEvent args)
    {
        /*
        if (HasComp<NukeOperativeComponent>(ent)) return; // Nuke Ops are immune to limb damage. Temporary solution. mb Death squad?
        if (!TryComp<HumanoidAppearanceComponent>(ent, out var appearance) || appearance.Species == "SlimePerson") return; // SlimePeople are immune to limb damage.
    ...
    private bool TryRemoveLimb(Entity<BodyComponent> ent, [NotNullWhen(true)] out Entity<BodyPartComponent>? part)
    {
            ...
            && virtualLimb.Item.HasValue)
        {
            ...
            var visualizer = EnsureComp<CustomLimbVisualizerComponent>(ent.Owner);

            var layer = SurgerySystem.GetLayer(slotId);
            if (layer is not null)
            {
                visualizer.Layers.Remove(layer.Value);
                Dirty(ent.Owner, visualizer);
            }
        }
        else
        {
